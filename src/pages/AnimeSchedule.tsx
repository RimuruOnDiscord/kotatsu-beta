...
import {
  ...
  Bookmark, BookmarkCheck, Clock, Building, Search, Infinity, X, SearchX, ServerCrash, Tv
} from 'lucide-react';
...
const fetchSchedule = async (page: number = 1): Promise<AnimeSearchResponse> => {
  // Added perPage=18 to the query string
  const res = await fetch(`/api/schedule?page=${page}&perPage=${ITEMS_PER_PAGE}`);
...
const DESIGN_STYLES = `
  ...
  .aw-pagination-btn {
    display: inline-flex; height: 36px; min-width: 36px; padding: 0 14px;
    align-items: center; justify-content: center; border-radius: 8px;
    background: rgba(255, 255, 255, 0.03); border: none;
    color: var(--aw-muted); font-family: var(--aw-font-display);
  ...
  .aw-pagination-btn.is-active { background: transparent; color: var(--aw-accent); }
  .aw-pagination-btn.is-active::after {
    content: ''; position: absolute; bottom: 4px; left: 30%; right: 30%;
    height: 2px; background: var(--aw-accent); border-radius: 4px;
  }
  .aw-pagination-btn:active:not(:disabled) { transform: scale(0.95); opacity: 0.7; }
  .aw-pagination-btn:disabled { opacity: 0.1; cursor: not-allowed; }

  .aw-skeleton-card {
    position: relative; background: var(--aw-s1); backdrop-filter: blur(12px);
    border: 1px solid var(--aw-border); border-radius: 14px; overflow: hidden;
  }
  .aw-skeleton-card::before {
    ...
    transform: translateX(-100%); animation: aw-shimmer 2s infinite ease-in-out;
  }
  @keyframes aw-shimmer { 100% { transform: translateX(100%); } }
      ...
      >
        ...
        <div className="relative z-10 flex flex-col p-4 flex-1 pointer-events-none">
          ...
          <div className="flex items-center justify-between mt-auto pt-3 transition-colors duration-150 group-hover:border-[color-mix(in_srgb,var(--aw-accent),transparent_70%)]">
            <span className="flex items-center gap-1.5 text-[11px] font-medium text-[var(--aw-accent)] truncate group-hover:text-[var(--aw-accent)] transition-colors duration-150" style={{ fontFamily: 'var(--aw-font-body)' }}>
              <Clock size={11} className="shrink-0" />
              <span className="truncate font-bold"><CountdownTimer initialSeconds={timeUntil} /></span>
            </span>
            <span className="text-[11px] font-bold text-zinc-500 group-hover:text-white transition-colors duration-150">
              EP {episode}
            </span>
          </div>
        </div>
...
const AnimeSchedule: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'list' | 'grid'>(() => (localStorage.getItem('scheduleViewMode') as 'list' | 'grid') || 'grid');
  ...
  const dayOptions = useMemo(() => [
    { value: 'ALL', label: 'All', dateNum: <Infinity size={18} strokeWidth={2} /> },
    ...getWeekDates()
  ], []);
  ...
  useEffect(() => {
    const loadSchedule = async () => {
      ...
      try {
        const data = await fetchSchedule(currentPage);
        setRawAnimeList(data.results || []);
        const totalPages = Math.ceil((data.total || 0) / ITEMS_PER_PAGE);
        setPageInfo({
          currentPage: data.page || currentPage, lastPage: totalPages || 1,
          hasNextPage: data.hasNextPage, total: data.total || 0
        });
  ...
  const displayAnimeList = useMemo(() => {
    ...
    const filtered = baseList.filter((anime) => {
      if (selectedDay === 'ALL') return true;

      const airingAt =
        anime.nextAiringEpisode?.airingAt ||
        (anime as any).airingAt;
  ...
  }, [rawAnimeList, searchResults, isSearching, selectedDay]);

  console.log(rawAnimeList.length);

  const contentKey = `${viewMode}-${currentPage}`;

  return (
    <div className="aw-root aw-noise relative min-h-screen overflow-x-hidden text-white selection:bg-[var(--aw-accent)]/20">
      <div style={{ position: 'sticky', top: 0, zIndex: 60, borderBottom: '1px solid color-mix(in srgb, var(--aw-border), transparent 30%)', background: 'color-mix(in srgb, var(--aw-bg), transparent 15%)', backdropFilter: 'blur(20px)' }}></div>
      ...
      <main className="mx-auto w-full max-w-[1460px] space-y-8 px-4 py-10 md:px-8 relative z-10">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="flex flex-col gap-2">
          <p className="aw-label flex items-center gap-2"><CalendarClock size={12} /> Airing Timetable</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }} className="flex w-full flex-col gap-5 lg:flex-row lg:items-center lg:justify-between border-b border-[color-mix(in_srgb,var(--aw-border),transparent_50%)] pb-6 lg:pb-5">

          {/* Left: Days Filter (Squircles perfectly proportioned to 46px height) */}
          <div className="flex min-w-0 flex-1 overflow-x-auto no-scrollbar items-center gap-2 pb-2 lg:pb-0">
            {dayOptions.map((day) => {
              ...
              return (
                <motion.button
                  ...
                  className={`group relative flex h-[46px] min-w-[56px] flex-col items-center justify-center rounded-[14px] outline-none transition-all duration-300 shrink-0 border ${isActive ? 'border-transparent' : 'border-[var(--aw-border)] bg-[var(--aw-s1)] hover:border-[color-mix(in_srgb,var(--aw-accent),transparent_70%)]'}`}
                >
                  {isActive && (
                    <motion.div
                      layoutId="scheduleActiveDaySquircle"
                      className="absolute inset-0 rounded-[14px] bg-[color-mix(in_srgb,var(--aw-accent),transparent_85%)] border border-[color-mix(in_srgb,var(--aw-accent),transparent_50%)] shadow-[0_4px_20px_-6px_color-mix(in_srgb,var(--aw-accent),transparent_70%)]"
                      transition={{ type: 'spring', damping: 20, stiffness: 300 }}
                    />
                  )}
                  <span className={`relative z-10 text-[8px] font-bold tracking-[0.1em] uppercase mb-[1px] transition-colors ${isActive ? 'text-white drop-shadow-sm' : 'text-zinc-500 group-hover:text-zinc-300'}`} style={{ fontFamily: 'var(--aw-font-display)' }}>
                    {day.label}
                  </span>
                  <span className={`relative z-10 text-[16px] font-medium leading-none flex items-center justify-center transition-colors ${isActive ? 'text-[var(--aw-accent)]' : 'text-zinc-400 group-hover:text-white'}`} style={{ fontFamily: 'var(--aw-font-body)' }}>
                    {day.dateNum}
                  </span>
                </motion.button>
              );
          ...
          <div className="flex shrink-0 flex-row items-center gap-3 w-full lg:w-auto">

            {/* Search Bar - Exactly 46px height */}
            <div className="flex h-[46px] w-full sm:w-[260px] xl:w-[300px] items-center gap-2 rounded-[14px] border border-[var(--aw-border)] bg-[var(--aw-s1)] px-3 backdrop-blur-md transition-all focus-within:border-[color-mix(in_srgb,var(--aw-accent),transparent_50%)] focus-within:shadow-[0_0_20px_color-mix(in_srgb,var(--aw-accent),transparent_90%)]">
              ...
              <input
                ...
                className="flex-1 min-w-0 bg-transparent text-[14px] text-white outline-none placeholder:text-zinc-500 font-medium"
                style={{ fontFamily: 'var(--aw-font-body)' }}
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="text-zinc-500 hover:text-white transition-colors shrink-0 outline-none"><X size={14} /></button>
              )}
            </div>

            {/* View Toggle - Exactly 46px height */}
            <div className="flex shrink-0 h-[46px] items-center gap-1 rounded-[14px] border border-[var(--aw-border)] bg-[var(--aw-s1)] p-1 backdrop-blur-md transition-colors duration-300">
              <motion.button
              ...
              {!isSearching && (
                <motion.div variants={itemVariants} className="flex flex-wrap items-center justify-center gap-2 pt-10 pb-6">
                  <motion.button
                    ...
                    type="button"
                    onClick={() => currentPage > 1 && commitPage(currentPage - 1)}
                  ...
                  <motion.button
                    ...
                    transition={{ duration: 0.1, ease: "easeInOut" }}
                    type="button"
                    onClick={() => pageInfo.hasNextPage && commitPage(currentPage + 1)}
