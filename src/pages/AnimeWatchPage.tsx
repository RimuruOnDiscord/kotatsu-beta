...
const DESIGN_STYLES = `
  ...
  @media (min-width: 1280px) {
    .aw-layout { grid-template-columns: 1fr 400px; gap: 32px; align-items: start; }
    .aw-sidebar { 
      height: auto;
      max-height: 850px;
...
const CustomScrollArea: React.FC<CustomScrollAreaProps> = ({ children, className, style, innerRef }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 0, right: 4, bottom: 0, width: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 100, pointerEvents: 'none' }}>
...
const AnimeWatch: React.FC = () => {
  ...
  useEffect(() => {
    // Only scroll if we have both the container and the active element
    const container = epListScrollRef.current;
    const activeEl = activeEpRef.current;
  ...
  }, [episodeId, loadingEpisodes, isSidebarCollapsed]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!data.intro && !data.outro && mounted) {
          // Priority background fetch for skip data from high-consistency providers
          const highConsistencyProviders = ['arc', 'kiwi', 'bee'];
          const remainingProviders = rankedProviders.filter(p => !highConsistencyProviders.includes(p.toLowerCase()));
          const prioritizedList = [...highConsistencyProviders.filter(p => rankedProviders.map(rp => rp.toLowerCase()).includes(p)), ...remainingProviders];

          for (const providerKey of prioritizedList) {
            ...
            try {
              ...
              if (matchEp) {
                ...
                const spData = await fetchAnimeStreams(providerKey.toLowerCase(), resolvedId, currentCategory as 'sub' | 'dub', spPure);
    ...
    load();
    return () => { mounted = false; };
  }, [currentEpData?.id, currentEpData?.number, resolvedId, currentProvider, currentCategory, episodeId, episodesData, rankedProviders, streamRetryNonce]);
  ...
  return (
    <div className="aw-root aw-noise min-h-screen flex flex-col relative">
      ...
      <motion.div
        ...
        animate="show"
      >
        <main className="aw-main" style={{ position: 'relative' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <motion.div
              ...
              style={{
                ...
                borderRadius: 16, overflow: 'hidden', zIndex: 10, transition: 'box-shadow 0.5s',
                boxShadow: lightsOff
                  ? '0 0 0 2px var(--aw-accent), 0 0 80px 8px var(--aw-accent), 0 30px 80px -20px rgba(0,0,0,0.9)'
                  : ytAmbientMode
                    ? '0 10px 30px -10px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.06)'
                ...
                {autoNextCountdown !== null && hasNext && (
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow: '0 20px 60px -18px rgba(0,0,0,0.85)',
                    }}
                  >
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontFamily: 'var(--aw-font-display)', fontSize: 12, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'white', whiteSpace: 'nowrap' }}>
                        Next episode in {autoNextCountdown} seconds
                      </div>
                      <div style={{ marginTop: 3, fontSize: 12, color: 'rgba(255,255,255,0.55)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 360 }}>
                        Episode {providerEpisodes[currentIndex + 1]?.number || '?'}: {providerEpisodes[currentIndex + 1]?.title || 'Up next'}
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setAutoNextCountdown(null)}
                      style={{ height: 34, padding: '0 14px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)', background: 'transparent', color: 'rgba(255,255,255,0.72)', fontSize: 11, fontFamily: 'var(--aw-font-display)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer' }}
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      style={{ height: 34, padding: '0 16px', borderRadius: 999, border: '1px solid var(--aw-accent)', background: 'var(--aw-accent)', color: '#04110d', fontSize: 11, fontFamily: 'var(--aw-font-display)', fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', whiteSpace: 'nowrap' }}
                    >
                      Play Now
                    </button>
                  </motion.div>
                ...
                >
                  ...
                  {useYouTubeStylePlayer && (
                    <div
                      ...
                      onClick={(e) => {
                        const target = e.target as HTMLElement;
                        if (target.closest('.yt-btn') || target.closest('.yt-volume') || target.closest('.yt-time-display')) return;
                        if (ytIsPlayerPaused) {
                          playerRef.current?.play();
                      ...
                      <AnimatePresence>
                        {ytControlsVisible && (
                          <motion.div
                            ...
                            transition={{ duration: 0.2 }}
                            onClick={(e) => e.stopPropagation()}
                            style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, rgba(0,0,0,0.6))', padding: '20px 16px 16px', zIndex: 105 }}
                          >
                            {(() => {
                              ...
                              return (
                                <div
                                  ref={ytProgressRef}
                                  style={{ padding: '14px 0 4px', marginBottom: 6, cursor: 'pointer', position: 'relative', touchAction: 'none' }}
                                  onPointerDown={(e) => {
                                    e.stopPropagation();
                                ...
                                >
                                  ...
                                  <div style={{ position: 'relative', height: 5, background: 'transparent', transformOrigin: 'center bottom', transform: isActive ? 'scaleY(1)' : 'scaleY(1)', transition: 'transform 0.14s cubic-bezier(0.4,0,0.2,1)', overflow: 'visible' }}>
                                    {segments.map(({ start, end }, i) => {
                                      ...
                                      return (
                                        <div key={i} style={{ position: 'absolute', top: 0, height: '100%', left: `${start}%`, width: `${segW}%`, background: 'rgba(255,255,255,0.2)', borderRadius: 2, overflow: 'hidden' }}>
                                          {fillPct > 0 && (
                                            <div style={{ width: `${fillPct}%`, height: '100%', background: 'var(--aw-accent)', borderRadius: 2, willChange: 'width' }} />
                                          )}
                                        </div>
                                      );
                                    })}
                                  ...
                                  <div style={{ position: 'absolute', top: 16, left: `${currentPct}%`, width: 14, height: 14, borderRadius: '50%', background: 'white', transform: `translate3d(-50%, -50%, 0) scale(${isActive ? 1 : 0})`, transition: 'transform 0.14s cubic-bezier(0.4,0,0.2,1)', boxShadow: '0 2px 10px rgba(0,0,0,0.5)', zIndex: 20, willChange: 'left, transform', pointerEvents: 'none' }} />
                                </div>
                              );
                            })()}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                {[
                                  { label: 'Play', tip: 'Play', icon: ytIsPlayerPaused ? <Play size={24} fill="white" /> : <Pause size={24} fill="white" />, action: (e: React.MouseEvent) => { e.stopPropagation(); if (ytIsPlayerPaused) { playerRef.current?.play(); setYtIsPlayerPaused(false); setYtPlayIndicator('play'); } else { playerRef.current?.pause(); setYtIsPlayerPaused(true); setYtPlayIndicator('pause'); } if (playIndicatorTimeoutRef.current) clearTimeout(playIndicatorTimeoutRef.current); playIndicatorTimeoutRef.current = setTimeout(() => setYtPlayIndicator(null), 600); } },
                                  { label: 'Next', tip: 'Next Episode', icon: <SkipForward size={20} fill="white" />, action: (e: React.MouseEvent) => { e.stopPropagation(); playNextEpisode(); } },
                                  ...
                                  <div key={label} style={{ position: 'relative' }}>
                                    <motion.button
                                      ...
                                      whileTap={{ scale: 0.88 }}
                                      onClick={action}
                                      style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.8)', cursor: 'pointer', padding: 10, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                    >
                                      {icon}
                                    </motion.button>
                                    <AnimatePresence>
                                      {ytTooltip === tip && (
                                        <motion.div
                                          ...
                                          style={{ position: 'absolute', bottom: 'calc(100% + 8px)', transformOrigin: 'bottom center', zIndex: 99999, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)', padding: '6px 12px', borderRadius: 8, color: 'white', fontSize: 12, fontFamily: 'var(--aw-font-display)', fontWeight: 600, whiteSpace: 'nowrap', pointerEvents: 'none', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 8px 24px rgba(0,0,0,0.5)', backfaceVisibility: 'hidden', left: '50%', x: '-50%' }}
                                        >
                                          {tip}
                                        </motion.div>
                                      )}
                                ...
                                <div onMouseEnter={() => setYtVolumeHover(true)} onMouseLeave={() => setYtVolumeHover(false)} style={{ display: 'flex', alignItems: 'center', gap: 0, position: 'relative' }} onClick={(e) => e.stopPropagation()}>
                                  <motion.div
                                    animate={{ width: ytVolumeHover ? 120 : 44, opacity: 1 }}
                                    style={{ overflow: ytVolumeHover ? 'visible' : 'hidden', display: 'flex', alignItems: 'center', background: ytVolumeHover ? 'rgba(255,255,255,0.06)' : 'transparent', borderRadius: 8, padding: ytVolumeHover ? '4px 6px' : '4px 10px', cursor: 'pointer', height: 44, position: 'relative' }}
                                    transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
                                  >
                                    <motion.button
                                      className="yt-btn"
                                      onMouseEnter={() => { if (ytTooltipTimerRef.current) clearTimeout(ytTooltipTimerRef.current); setYtTooltip(ytIsMuted ? 'Unmute' : 'Mute'); }}
                                      onMouseLeave={() => { ytTooltipTimerRef.current = setTimeout(() => setYtTooltip(null), 50); }}
                                      whileTap={{ scale: 0.88 }}
                                      onClick={() => { setYtIsMuted(!ytIsMuted); if (playerRef.current) playerRef.current.muted = !ytIsMuted; }}
                                      style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.8)', cursor: 'pointer', padding: 10, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                                    >
                                      {ytIsMuted ? <VolumeX size={20} /> : <Volume2 size={20} />}
                                    </motion.button>
                                    <AnimatePresence>
                                      {(ytTooltip === 'Mute' || ytTooltip === 'Unmute') && (
                                        <motion.div
                                          initial={{ opacity: 0, y: 4, scale: 0.92 }}
                                          animate={{ opacity: 1, y: 0, scale: 1 }}
                                          exit={{ opacity: 0, y: 4, scale: 0.92 }}
                                          transition={{ duration: 0.15, ease: [0.16, 1, 0.3, 1] }}
                                          style={{ position: 'absolute', bottom: 'calc(100% + 8px)', transformOrigin: 'bottom center', zIndex: 99999, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)', padding: '6px 12px', borderRadius: 8, color: 'white', fontSize: 12, fontFamily: 'var(--aw-font-display)', fontWeight: 600, whiteSpace: 'nowrap', pointerEvents: 'none', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 8px 24px rgba(0,0,0,0.5)', backfaceVisibility: 'hidden', left: 22, x: '-50%' }}
                                        >
                                          {ytTooltip}
                                        </motion.div>
                                      )}
                                ...
                                <button
                                  ...
                                  style={{ color: 'white', fontSize: 14, fontFamily: 'var(--aw-font-display)', marginLeft: 12, fontWeight: 600, cursor: 'pointer', userSelect: 'none', padding: '4px 10px', borderRadius: 6, display: 'flex', alignItems: 'center', background: 'none', border: 'none' }}
                                >
                                  {ytShowTimeLeft === 0 ? (
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 2, position: 'relative' }}>
                                {[
                                  { label: 'Settings', tip: 'Settings', icon: (active: boolean) => active ? <X size={20} /> : <Settings size={20} />, active: ytShowSettingsMenu, action: (e: React.MouseEvent) => { e.stopPropagation(); setYtShowSettingsMenu(!ytShowSettingsMenu); } },
                                  {
                                    label: 'Captions', tip: ytShowCaptions ? 'Captions On' : 'Captions', icon: (active: boolean) => active ? (
                                      <svg width={20} height={20} viewBox="0 0 24 24" fill="white"><rect x="3" y="5" width="18" height="14" rx="2" fill="white" /><path d="M7 15h4M15 15h2M7 11h2M13 11h4" stroke="black" strokeWidth="1.5" strokeLinecap="round" fill="none" /></svg>
                                    ) : (
                                      <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="2" strokeLinecap="round"><rect x="3" y="5" width="18" height="14" rx="2" /><path d="M7 15h4M15 15h2M7 11h2M13 11h4" /></svg>
                                ...
                                <AnimatePresence>
                                  {ytShowSettingsMenu && (
                                    <motion.div
                                      ...
                                      style={{ position: 'absolute', bottom: 'calc(100% + 16px)', right: 0, background: 'rgba(10,10,15,0.7)', backdropFilter: 'blur(16px) saturate(120%)', border: '1px solid var(--aw-border)', borderRadius: 12, width: 280, boxShadow: '0 20px 60px rgba(0,0,0,0.8)', zIndex: 99999, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
                                      onClick={e => e.stopPropagation()}
                                    >
                                      <style>{`
                  ...
                  <style>{`
                    .yt-time-display:hover { background: rgba(255,255,255,0.1) !important; }
                  ...
                  <AnimatePresence>
                    {seekIndicator && (
                      <motion.div
                        ...
                        transition={{ type: 'spring', stiffness: 400, damping: 25 }}
                        style={{ position: 'absolute', top: '50%', marginTop: -40, left: seekIndicator === 'rewind' ? 20 : 'auto', right: seekIndicator === 'forward' ? 20 : 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, color: 'white', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: '14px 18px', zIndex: 150, pointerEvents: 'none' }}
                      >
                        {currentEpData?.image && (
          ...
          <div style={{ padding: '0px 0 0', position: 'relative', zIndex: 100 }}>
            {/* Unified Interaction Hub (Title + Controls + Description) */}
            <motion.div
              variants={fadeUpItem}
              className="modern-card watch-description-frame"
              style={{ padding: 32, marginBottom: 24, position: 'relative', zIndex: 100 }}
            >
              {/* 1. Integrated Title & All Controls Row */}
              <motion.div
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, gap: 32, position: 'relative', zIndex: 100 }}
              >
                <h1 style={{ margin: 0, fontSize: 22, fontFamily: 'var(--aw-font-display)', fontWeight: 600, color: 'white', letterSpacing: '-0.01em', flex: 1, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {currentEpData?.title || 'Loading...'}
                  <span style={{ color: 'var(--aw-accent)', margin: '0 12px', opacity: 0.6 }}>•</span>
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>Episode {currentEpData?.number || '?'}</span>
                </h1>
                <div className="watch-controls">

                  {/* Container 1: Action Tools */}
                  <motion.div
                    className="watch-control-group"
                  >
                    {/* Bookmark Button */}
                    <motion.button
                      whileTap={{ scale: 0.96 }}
                  ...
                  {/* Container 2: Playback Settings */}
                  <motion.div
                    className="watch-control-group playback"
                  >
                    {/* Mode Selector */}
                    <motion.div
                    ...
                    <motion.div
                      onClick={() => {
                        const nextLang = (category || 'sub') === 'sub' ? 'dub' : 'sub';
                        const isAvailable = streamData?.sources?.some(s => s.type === nextLang) || (nextLang === 'sub');
                        if (isAvailable) {
                          navigate(`/watch/${urlSlug}/${provider || 'default'}/${nextLang}/${episodeId}`);
                        }
                      }}
                      whileTap={{ scale: 0.97 }}
                      transition={{ type: 'spring', stiffness: 420, damping: 22 }}
              ...
              <div style={{ position: 'relative' }}>
                <p
                  className="watch-description-copy"
                  style={{
                    margin: 0,
                    fontSize: 15,
                    lineHeight: '1.8',
                ...
                >
                  {animeInfo?.description?.replace(/<[^>]*>/g, '') || 'No description available for this series.'}
                </p>

                <motion.button
                  whileHover={{ x: 4, color: 'var(--aw-accent)' }}
                  onClick={() => setIsDescExpanded(!isDescExpanded)}
                  className="watch-description-toggle"
                  style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.8)', fontWeight: 700, fontSize: 13, cursor: 'pointer', padding: '14px 0 0', display: 'block', transition: 'color 0.2s', fontFamily: 'var(--aw-font-display)' }}
                >
                  {isDescExpanded ? 'Show less' : 'Show more'}
                </motion.button>
              </div>

              {/* YouTube-Style "Series" attribution block */}
            </motion.div>
          </div>
        ...
        <aside className={`aw-sidebar sidebar-enter ${isSidebarCollapsed ? 'collapsed' : ''}`} style={{ zIndex: 40 }}>
          <div style={{ position: 'relative', zIndex: 50, padding: '18px 20px', background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(8px)', borderBottom: '1px solid var(--aw-border)', flexShrink: 0, overflow: 'visible' }}>
            <div className="anim-fade-in-down" style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div style={{ position: 'relative' }} ref={seasonsDropdownRef}>
                <button
                  onClick={() => { if (navTabs.length > 1) { setIsSeasonsDropdownOpen(p => !p); setIsOptionsMenuOpen(false); } }}
                  className={navTabs.length > 1 ? "aw-action-hover" : ""}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: 'none', padding: '4px 8px', marginLeft: '-8px', borderRadius: 8, color: 'white', cursor: navTabs.length > 1 ? 'pointer' : 'default', outline: 'none' }}
                >
                  ...
                  {navTabs.length > 1 && (
                    <ChevronDown size={18} style={{ opacity: 0.8, transform: isSeasonsDropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s' }} />
                  )}
                </button>
                ...
                <AnimatePresence>
                  {isSeasonsDropdownOpen && (
                    <motion.div
                      ...
                      style={{ position: 'absolute', top: '100%', left: 0, marginTop: 12, width: 280, background: 'var(--app-bg)', borderRadius: 12, padding: 8, zIndex: 100, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.07)' }}
                    >
                      {navTabs.map((tab, i) => (
                        <motion.button
                          key={tab.id}
                          ...
                          onClick={() => {
                            setIsSeasonsDropdownOpen(false);
                            const firstEpId = (provider === 'animepahe' || provider === 'kiwi') ? 'animepahe-1' : '1';
                            navigate(`/watch/${tab.slug}/${provider || 'default'}/${category || 'sub'}/${firstEpId}`);
                          }}
                          style={{ position: 'relative', width: '100%', textAlign: 'left', padding: '12px 14px', background: tab.active ? 'color-mix(in srgb, var(--aw-accent) 15%, transparent)' : 'transparent', border: 'none', borderRadius: 8, cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 6, transition: 'background 0.2s', overflow: 'visible' }}
                          onMouseEnter={e => !tab.active && (e.currentTarget.style.background = 'rgba(255,255,255,0.04)')}
                          onMouseLeave={e => !tab.active && (e.currentTarget.style.background = 'transparent')}
                        >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'rgba(255,255,255,0.6)', position: 'relative' }} ref={optionsMenuRef}>
                <button
                  title="Options"
                  onClick={() => { setIsOptionsMenuOpen(o => !o); setIsSeasonsDropdownOpen(false); }}
                  className="aw-action-hover"
                  style={{ background: isOptionsMenuOpen ? 'rgba(255,255,255,0.08)' : 'transparent', border: 'none', padding: 6, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: isOptionsMenuOpen ? 'white' : 'inherit', transition: 'all 0.2s' }}
                >
                  <MoreVertical size={16} />
                </button>
                <button
                  title={isSidebarCollapsed ? 'Show episodes' : 'Hide episodes'}
                  onClick={() => setIsSidebarCollapsed(p => !p)}
                  className="aw-action-hover"
                  style={{ background: 'transparent', border: 'none', padding: 6, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'inherit' }}
                >
                  <ChevronDown size={16} style={{ transform: isSidebarCollapsed ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.3s' }} />
                </button>
                ...
                <AnimatePresence>
                  {isOptionsMenuOpen && (
                    <motion.div
                      ...
                      style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, width: 200, background: 'var(--app-bg)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: 6, zIndex: 200, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}
                    >
                      {[
                        ...
                        <button
                          key={item.label}
                          onClick={() => { item.action(); setIsOptionsMenuOpen(false); }}
                          style={{ width: '100%', textAlign: 'left', padding: '9px 12px', background: 'transparent', border: 'none', borderRadius: 7, cursor: 'pointer', color: 'rgba(255,255,255,0.8)', fontSize: 12, fontFamily: 'var(--aw-font-body)', transition: 'background 0.15s' }}
            ...
            <div className="anim-fade-in-up anim-delay-1" style={{ position: 'relative', zIndex: 1, display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              >
                <motion.input
                  ...
                  style={{
                    width: '100%', height: 38, background: 'rgba(255,255,255,0.02)',
                    border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10,
                    padding: '0 14px', color: 'var(--aw-text)', fontSize: 12,
                    fontFamily: 'var(--aw-font-body)', fontWeight: 400, outline: 'none',
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'flex-end', flexShrink: 0 }}>
                <motion.button
                  ...
                  style={{
                    ...
                    width: 38, height: 38, flexShrink: 0, background: 'rgba(255,255,255,0.02)',
                    border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10,
                    color: 'var(--aw-muted)', cursor: 'pointer', outline: 'none',
                    boxSizing: 'border-box'
                  }}
                >
                  <motion.div
                    animate={{ rotate: episodeSortOrder === 'desc' ? 0 : 180 }}
                    transition={{ type: 'spring', stiffness: 300, damping: 15 }}
                  >
                    <ArrowDownUp size={14} />
                  </motion.div>
              ...
              <CustomScrollArea innerRef={epListScrollRef} style={{ flex: 1, height: '100%' }}>
                {loadingEpisodes ? (
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {[...Array(8)].map((_, i) => (
                      <div key={i} style={{ display: 'flex', gap: 14, padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        <div className="skeleton-wave" style={{ width: 28, height: 64, borderRadius: 6, flexShrink: 0 }} />
                        <div className="skeleton-wave" style={{ width: 110, height: 64, borderRadius: 10, flexShrink: 0, animationDelay: '0.1s' }} />
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 6 }}>
                          <div className="skeleton-wave" style={{ width: '70%', height: 14, borderRadius: 4, animationDelay: '0.2s' }} />
                          <div className="skeleton-wave" style={{ width: '90%', height: 12, borderRadius: 4, animationDelay: '0.3s' }} />
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {visibleEpisodes.map((ep, idx) => {
                      ...
                      return (
                        <motion.div
                          ...
                          animate={{ opacity: 1, x: 0 }}
                          transition={{ duration: 0.4, delay: Math.min(idx * 0.03, 0.6), ease: [0.23, 1, 0.32, 1] }}
