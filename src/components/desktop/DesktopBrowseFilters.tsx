...
import {
  Search, Tags, Activity, Globe, Calendar,
  ...
  return (
    <div className={`relative ${isOpen ? 'z-[100]' : 'z-10'}`} ref={ref}>
      <motion.button
        ...
        className={`group relative flex h-[45px] select-none items-center gap-2.5 rounded-[12px] px-4 text-[13.5px] font-bold outline-none overflow-hidden border transition-all duration-150 backdrop-blur-md
          ${isOpen || hasSelection
            ? 'border-[var(--app-accent)] bg-[color-mix(in_srgb,var(--app-accent),transparent_92%)] text-[var(--app-accent)]'
            : 'border-white/[0.06] bg-transparent text-zinc-400 hover:border-[color-mix(in_srgb,var(--app-accent),transparent_55%)] hover:bg-[color-mix(in_srgb,var(--app-accent),transparent_95%)] hover:text-white'
          }`}
      >
        ...
        {isOpen && (
          <div className="absolute top-[calc(100%+8px)] left-1/2 -translate-x-1/2 z-[100] min-w-[200px]" style={{ transformOrigin: 'top center' }}>
            <motion.div
              ...
              className="overflow-hidden rounded-[14px] border border-white/[0.08] bg-[#18181b]/95 p-1.5 shadow-[0_16px_40px_-8px_rgba(0,0,0,0.8),0_0_0_1px_color-mix(in_srgb,var(--app-accent),transparent_90%)] backdrop-blur-xl"
            >
...
export default function DesktopBrowseFilters(props: DesktopBrowseFiltersProps) {
  ...
  return (
    <motion.div layout className="hidden xl:flex flex-wrap items-center gap-2.5 w-full">
      ...
      <motion.div
        layout
        className="group relative flex min-w-[260px] max-w-[320px] flex-1 items-center h-[45px] rounded-[12px] border border-white/[0.06] transition-all duration-150 focus-within:border-[var(--app-accent)] focus-within:shadow-[0_0_16px_color-mix(in_srgb,var(--app-accent),transparent_80%)] overflow-hidden"
        style={{
      ...
      >
        <div className="flex shrink-0 items-center justify-center h-full w-[44px] text-zinc-500 group-focus-within:text-[var(--app-accent)] transition-colors duration-150 relative z-10">
          <Search size={15} />
        </div>
        <input
          ...
          placeholder={props.searchPlaceholder || 'Search Anime...'}
          className="flex-1 bg-transparent text-[13.5px] font-medium text-white placeholder:text-zinc-500 border-none outline-none ring-0 h-full pr-14 relative z-10"
          style={{ fontFamily: 'var(--aw-font-body)' }}
        />

        <div className="absolute right-2 top-0 bottom-0 flex items-center justify-center z-10 gap-1.5">
          <AnimatePresence>
            {props.searchQuery && (
              <motion.button
      ...
      <motion.button
        ...
        className={`ml-1 flex h-[45px] shrink-0 items-center gap-2 rounded-[12px] px-4 text-[11.5px] font-bold tracking-widest text-red-400 bg-red-500/[0.08] border border-red-500/20 hover:bg-red-500/[0.12] hover:border-red-500/30 hover:text-red-300 overflow-hidden whitespace-nowrap transition-all duration-150 ${props.hasActiveFilters ? 'pointer-events-auto' : 'pointer-events-none'
          }`}
